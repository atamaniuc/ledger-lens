...
// The gate `app/globals.css` and `components/ui/status-badge.tsx` both claim
...
//
// Scoped to what renders: `app/` and `components/`. `globals.css` is where
...
describe("design tokens", () => {
  it("scans the files that actually render", () => {
    ...
    expect(files).toContain("components/ui/status-badge.tsx");
  });
