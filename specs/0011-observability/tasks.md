...
- [x] **T1** OTel SDK in `src/platform/obs/`; traces across ingest → transform → quality → agent (D-45)
- [x] **T2** `correlation_id` as trace id end-to-end; test asserts one trace per request (D-45)
- [x] **T3** 4 metrics: `freshness_lag`, `ingest_error_rate`, `agent_p95_latency`, `llm_daily_cost` (D-45)
- [x] **T4** 2 alerts: freshness > N, cost > cap (D-45)
