...
export type StepOutcome =
  ...
  | "timeout"
  | "token_ceiling"
  // The client went away. Distinct from "error" on purpose: a cancelled turn
  // must never be counted as an answer, and the column is what an eval or a
  // cost report reads (spec 0013).
  | "cancelled"
  | "error";
...
export interface LlmCallRecord {
  ...
  outcome: StepOutcome;
  /**
   * Which provider answered (ADR 0010). Empty for a row that records a turn
   * a bound ended, where no call was made at all.
   */
  provider?: string;
  /**
   * The chain head at write time, so a fallback rate is derivable from the
   * table alone (one query: provider <> preferred_provider).
   */
...
export async function logLlmCall(
  ...
  const { data, error } = await supabase.rpc("log_llm_call", {
    ...
    // ADR 0010: who answered, and which chain head was preferred at write
    // time — the two columns the fallback-rate query reads.
    p_provider: record.provider ?? "",
    p_preferred_provider: record.preferredProvider ?? record.provider ?? "",
  });
