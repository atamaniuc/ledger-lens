...
export interface RetrievedContext {
  ...
  /**
   * The internal row ids of the same invoices, when a tool result carried them.
   *
   * A measured eval run showed the model citing `[invoice:<uuid>]`, because
   * `list_invoices` hands it a field called `invoice_id` — and every one of
   * those citations came back unverified even though it named an invoice the
   * tool really had returned. The prompt asks for the external id and the UI
   * shows that, but refusing to recognise an id this system itself supplied was
   * the verifier being wrong, not the model (D-25).
   */
  invoiceRowIds?: readonly string[];
}
...
export function verifyCitations(answer: string, context: RetrievedContext): CitationCheck {
  ...
  const invoiceIds = new Set([
    ...context.invoiceExternalIds,
    ...(context.invoiceRowIds ?? []),
  ]);
