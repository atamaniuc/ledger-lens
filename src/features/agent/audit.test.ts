...
import { MODEL_PRICES, costCents } from "./pricing";
...
describe("costCents", () => {
  ...
  it("prices every model a configured provider would reach for by default", () => {
    // Asserted against the provider specs rather than a local constant: the
    // constant that used to live in this module was a second source of truth
    // for "the default model" and only this test read it (D-41).
    //
    // `openai-compatible` is the deliberate exception — it has no default
    // model because the whole point of that provider is that the operator names
    // one (`LLM_MODEL`), and an unpriced model costs a null in
    // `llm_calls.cost_cents`, which is the honest answer to "what did that
    // turn cost" rather than a fabricated zero.
    for (const spec of PROVIDERS.filter((p) => p.defaultModel !== "")) {
      expect(MODEL_PRICES[spec.defaultModel], spec.defaultModel).toBeDefined();
    }
  });
