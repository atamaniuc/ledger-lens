...
export const PROVIDERS: ProviderSpec[] = [
  ...
  },
  {
    ...
    // Verified against NVIDIA's catalogue on 2026-08-19: this default answers
    // a tool-calling request. Like Groq's, it must be a chat model that
    // supports tool calling — an embedding model has no /chat/completions
    // route at all — and NVIDIA's ids are namespaced, so a bare model name
    // 404s even when the model exists.
...
export function configured(spec: ProviderSpec): ResolvedProvider | null {
...
/**
 * One already-resolved provider, wrapped in the agent's narrow client
 * interface. Anthropic uses its SDK; every other provider in the spec table
 * speaks chat-completions and goes through the OpenAI-compatible adapter.
 */
export function clientFor(resolved: ResolvedProvider): ModelClient {
