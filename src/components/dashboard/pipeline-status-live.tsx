...
import type { RunSummary } from "@/features/dashboard/queries";
...
export function PipelineStatusLive({
  runs,
  error = null,
  isLoading = false,
}: {
  runs: RunSummary[];
  /** When the runs query failed. The panel renders its own error then. */
  error?: string | null;
  isLoading?: boolean;
}) {
  const { state } = useLive();
  ...
  if (isLoading) {
    return <PanelSkeleton label="Pipeline runs loading" lines={3} />;
  }
  ...
  return (
    <Panel
      ...
      action={
        // The connection state changes asynchronously; announce it.
        state === "live" ? (
    ...
    >
      {error ? (
        ...
        <EmptyState>
          No pipeline runs yet. Trigger an ingestion run and it will appear
