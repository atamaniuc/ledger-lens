import type { DataHealth, QueryResult } from "@/features/dashboard/queries";
...
export function DataHealthPanel({
  ...
  result: QueryResult<DataHealth>;
  isLoading?: boolean;
}) {
  if (isLoading) {
    return <PanelSkeleton label="Data health loading" lines={4} />;
  }
  ...
  if (!run) {
    return (
      <Panel title="Data health" testId="data-health">
        <EmptyState>
          No pipeline run has completed yet. Trigger an ingestion run — the
          quality checks for it will appear here as soon as it closes.
        </EmptyState>
      </Panel>
    );
  }
