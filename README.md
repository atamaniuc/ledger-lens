...
| citation validity | **0.83 against a 0.95 bar — still red, and the bar stays.** The number is a diagnosis log: 0.23 → 0.80 → 0.83 across two measured runs. 0.23 was structural (more than half these cases ask for a revenue figure, and the tool returned a total with nothing to cite, so a perfect answer could not clear 0.47); the next run showed the model citing the internal uuid the tools handed it, which the verifier refused — its mistake, not the model's. What is left is two uncited answers out of thirty <!-- proof: evals/thresholds.json --> <!-- proof: src/features/agent/tools/get-revenue-summary.ts:MAX_EVIDENCE_IDS --> |
